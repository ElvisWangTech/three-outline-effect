import { BufferGeometry, Vector3, BufferAttribute, Triangle, EdgesGeometry } from 'three';
...
export class ConditionalEdgesGeometry extends EdgesGeometry {

	constructor( geometry: BufferGeometry ) {

		super();

		const edgeInfo: Record<string, any> = {};
		...
		if ( geometry.index ) {
			...
			const arr = new Array( position.count / 3 ).fill(undefined).map( ( _, i ) => i );
